...
module.exports = (defaultFuncs, api, ctx) => {
  ...
  return function getUserInfo(id, usePayload, callback, groupFields = []) {
    ...
    // Validate user IDs
    if (!ctx.validator.validateIDArray(ids, ctx.validator.isValidUserID.bind(ctx.validator))) {
      return callback(new Error("Invalid user ID(s)"));
    }
      ...
      Promise.all(ids.map(fetchProfile))
        .then(results => {
          ...
          ids.forEach((id, index) => {
            ctx.cache.set(`user_${id}`, results[index]);
          });
